...
$dql = new connectDB();
if(isset($_GET['mahd']) && $_GET['mahd'] !== ''&&isset($_GET['makh']) && $_GET['makh'] !== ''){
    $maHD = $_GET['mahd'];
    $makh = $_GET['makh'];
    $sql = "SELECT DISTINCT TenKH,khachhang.SĐT as SDT,khachhang.DIACHI as Diachi,hoadon.TongTien as Tongtien from ct_hd,hoadon,khachhang,sanpham where ct_hd.MaHD=hoadon.MaHD and ct_hd.MaSP=sanpham.MaSP  and hoadon.MaKH=khachhang.MaKH and khachhang.MaKH='$makh' and hoadon.MaHD=$maHD and hoadon.hienthi=1";  
    $result = $dql->query($sql);
    $sql2 = "SELECT DISTINCT TenSP,Hinhanh,ct_hd.Gia as Gia,ct_hd.soluong as soluong from ct_hd,hoadon,khachhang,sanpham where ct_hd.MaHD=hoadon.MaHD and ct_hd.MaSP=sanpham.MaSP and hoadon.MaKH=khachhang.MaKH   and hoadon.MaKH=khachhang.MaKH and khachhang.MaKH='$makh' and hoadon.MaHD=$maHD and hoadon.hienthi=1";  
                ...
                foreach ($result as $key => $value) {
                    ...
                    <div class="abcd"><ul class="abcddsdonhang">';
                        ...
                        foreach ($result2 as $key2 => $value2) {
                            ...
                            <li class="sanpham">
                                ...
                                <div class="SP-info">
                                    ...
                                    <div class="Sp-gia-sl">'.numberToVND($value2["Gia"]).'<span> X '.$value2["soluong"].' cái</span></div>
                                </div>
                            </li>';
                        }
                        ...
                        $output .= '</ul></div>
                            ...
                            <input type="text" value="'.numberToVND($value["Tongtien"]).'">
                            <br/>
                        </div>
                        </form>
                        </div>';
