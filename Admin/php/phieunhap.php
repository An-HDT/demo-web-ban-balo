            <form id="date-form" action="hienthiPN.php" method="get">
                <div class="date">
            ...
            <div class="formabc-2" id="formtinhtrang" style="display: none;height:200px;">
                <form id="tinhtrang-form" action="updateTT.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="abcd"><label>Mã hóa đơn</label>
                        <input type="text" id="mahd" name="mahd" value="">
                        <br/>
                    </div>
                    <div class="abcd"> 
  ...
  <script>
              $('#date-form').submit(function(event) {
                ...
                function showCTNH(mapn) {
                  event.preventDefault();
                    var url = 'hienthiCTPN.php?mapn=' + mapn;
                    $.ajax({
                      url: url,
                      type: 'GET',
                      success: function(response) {
                  ...
                  function deleteNH(mapn) {
                    var result = confirm("Bạn có chắc chắn muốn xóa không ???");
                    if (result) {
                        event.preventDefault(); // ngăn chặn hành động mặc định của trình duyệt
                        var url = 'deletePN.php?mapn=' + mapn;
                        $.ajax({
                            url: url,
                            type: 'GET',
                            success: function(response) {
                              alert(response);
                              $('#danhsachHD').load('hienthiPN.php');
                            },
                            error: function() {
                            }
                        });
                ...
                function showNH(page) {
                    var start_date=document.getElementById("start_date").value;
                    var end_date=document.getElementById("end_date").value;
                    event.preventDefault();
                      var url = 'hienthiPN.php?sotrang='+ page+'&start_date='+start_date +'&end_date='+end_date;
                      console.log(url);
                      $.ajax({
                        url: url,
                        type: 'GET',
